...
/** An admin email template in the component's vocabulary (`pKey` / `bodyContent`). */
...
/** The backend DTO shape (`templateKey` / `detailContent`); mapped to/from {@link AdminEmailTemplate}. */
...
/**
 * Admin Email Templates API client — targets `api/AdminEmailTemplates`.
 *
...
export class AdminEmailTemplatesService {
  ...
  private readonly apiBase = `${environment.apiBaseUrl}/api/AdminEmailTemplates`;

  /** GET — list all manageable admin email templates. */
  getAll(): Observable<AdminEmailTemplate[]> {
    return this.http
  ...
  create(template: Omit<AdminEmailTemplate, 'pKey'>): Observable<AdminEmailTemplate> {
    return this.http
  ...
  update(pKey: number, template: Omit<AdminEmailTemplate, 'pKey'>): Observable<AdminEmailTemplate> {
    return this.http
...
function toModel(a: ApiTemplate): AdminEmailTemplate {
  return {
    pKey: a.templateKey,
...
function toApi(m: Omit<AdminEmailTemplate, 'pKey'>): Omit<ApiTemplate, 'templateKey'> {
  return {
